...
def cancel_expired_orders():
    ...
    for order in orders:
        ...
        cancelled_count += 1
        logger.info(
            "Order cancelled. order_number=%s",
            order.order_number,
        )
    logger.info("Expired order cancellation completed. count=%s", cancelled_count)
...
def generate_invoice_pdf(order_id):
    ...
    filename = f"/tmp/invoice_" f"{order.order_number}.pdf"
