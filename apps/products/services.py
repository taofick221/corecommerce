...
def create_product(validated_data):
    ...
    skus = [(v.get("sku") or "").strip().upper() for v in variants_data]

    if len(skus) != len(set(skus)):
        raise ValidationError({"sku": "Duplicate SKU found"})
    ...
    if len(combos) != len(set(combos)):
        raise ValidationError({"variant": "Duplicate size+color variant"})
    ...
    try:
        ...
        raise ValidationError(
            {"detail": "Duplicate SKU, variant conflict, or image validation failed"}
        )
    return product
